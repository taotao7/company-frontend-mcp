...
const PAGE_GUIDELINES = {
  ...
  apiGuidelines: {
    ...
    requestWrapper:
      "request(url: string, method: Method, data?: any, opts?: any)",
    listApiResponse: {
      structure: `{
  "code": 0,
  "msg": "string",
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "generate_page_template": {
      ...
      switch (type) {
        case "page":
          template = PAGE_TEMPLATES.basicPage.replace(
            /\[PageName\]/g,
            pascalName
          );
          break;
      ...
      switch (type) {
        ...
        case "api":
          ...
          if (
            ...
            issues.push(
              "❌ API函数命名不符合规范（应包含List、add、update、del等）"
            );
          }
    ...
    case "get_naming_suggestions": {
      const { description, type } = args as {
        description: string;
        type: string;
      };
